...
**a)** Training and validation data corresponding to 7 types of hand gestures is collected using the ```collection.py``` script. For example, data for label ```W``` can be collected by executing the following command,
...
**d)** Once images corresponding to label W are collected, then the user must go to the folder where those images are saved, and there user must select which images to keep and which ones to discard/delete. Once not-so-good images corresponding to label W are deleted, the user is supposed to re-run the same above command of ```python collecting.py W```to capture more images to replace the deleted garbage images of label W. 
...
After collecting the custom dataset locally, run the ```TalkingHand/training.ipynb``` script from your local via Google Colab and follow each instruction in that script. 

This script will clone this Github repo, ask you (the user) to upload the custom dataset (located in the local system) and train the underlying model. After training, the weights file gets saved in the ```TalkingHand``` folder (present in Google Drive), and then we can use it locally for inference (exact like we did in section 1.2.
